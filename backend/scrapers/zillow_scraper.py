"""Zillow scraper for land/property listings."""
import time
...
class ZillowScraper(BaseScraper):
    """Scrape Zillow for land/property comparables."""
    ...
    def scrape(self, search_query: str) -> list[ComparableListing]:
        ...
        cache_key = self._cache_key(
            f"https://www.zillow.com/homes/{search_query}",
            search_query
        )
        ...
        listings = []
        try:
            url = f"https://www.zillow.com/homes/{search_query.replace(' ', '-')}_rb/"
            ...
            cards = soup.select('[data-test="property-card"]') or soup.select('article')
    ...
    def _parse_card(self, card, base_url: str) -> Optional[ComparableListing]:
        ...
        try:
            name_el = card.select_one('a[data-test="property-card-link"]') or card.select_one('h2') or card.select_one('a')
            data["name"] = name_el.get_text(strip=True)[:100] if name_el else "Zillow Listing"
            ...
            if price_el:
                ...
                if price and price < 10000:  # Likely nightly
                    data["price_per_night"] = price
                elif price:
                    data["price_per_night"] = price / 365 * 0.6  # Est. nightly from sale price
    ...
    def _get_mock_results(self, search_query: str) -> list[ComparableListing]:
        return [
            self._to_listing({
                "name": f"Land for sale - {search_query}",
                "price_per_night": 0,  # Sale listing - use for comps
